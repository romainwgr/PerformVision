$(document).ready(function() {
    ...
    $('.btn').click(function(event) {
        ...
        var nom = $('#nom').val();
        var email = $('#email-prestataire').val();
        var tel = $('#tel-prestataire').val();
        ...
        if (!isValidEmail(email)) {
            $('#email-prestataire').css('border', '1px solid red');
            $('#email-error').text('Adresse email non valide.').show();
            isValid = false;
        }

        if (!isValidPhoneNumber(tel)) {
            $('#tel-prestataire').css('border', '1px solid red');
        ...
        $.ajax({
            url: 'index.php?controller=gestionnaire&action=ajout_prestataire',
            type: 'POST',
            data: data,
            dataType: 'json',
